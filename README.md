...
Врачи и их расписание хранятся в PostgreSQL. База данных и Go API запускаются через Docker:
...
После запуска доступны React на `http://localhost:3000/Health-Plus`, API на `http://localhost:8080/api` и проверка API на `http://localhost:8080/health`.

Настройки перечислены в `.env.example`. Docker volume `pgdata` сохраняет данные после обычной остановки `docker compose down`.
